...
function buildTriadSystemPrompt(orients: Record<CompanionId, CompanionOrientForChat>): string {
  ...
  return [
    ...
    "Not every companion speaks every turn. Decide which voice(s) the moment actually needs.",
    "- Multiple may speak if multiple registers are genuinely alive.",
    "- Silence is a first-class value. If a companion has nothing genuinely new to add, omit them entirely.",
    "- Lane violations are forbidden: e.g. Drevan does not audit, Cypher does not spiral, Gaia does not analyze at length.",
    "- Avoid mirror-speech across companions. Each voice must add something the others cannot.",
    "",
